      ...
      <div className="rounded-2xl bg-moss/8 border border-moss/20 p-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <a href="/studio" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-ink hover:bg-charcoal text-cream font-display font-bold text-[14px] uppercase tracking-[0.1em] px-6 py-3 rounded-full transition-all">
            Open Sanity Studio
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
          </a>
          <Link href={`/blog/${slug}`}
            className="inline-flex items-center gap-2 border border-line dark-flip-border hover:border-crimson text-charcoal dark-flip-text hover:text-crimson font-display font-bold text-[14px] uppercase tracking-[0.1em] px-6 py-3 rounded-full transition-all">
            Preview Article
          </Link>
          <button onClick={() => { setStatus('idle'); setSlug('') }}
            className="font-sans text-[14px] text-charcoal/45 dark-flip-muted hover:text-charcoal/70 transition-colors">
            Create another
          </button>
        </div>
      </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {errMsg && (
        <div className="bg-crimson/8 border border-crimson/25 rounded-xl px-4 py-3">
          <p className="font-sans text-[14px] text-crimson">{errMsg}</p>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className="font-display font-semibold text-[14px] uppercase tracking-[0.1em] text-charcoal/50 dark-flip-muted block mb-2">Category</label>
        ...
        <div>
          <label className="font-display font-semibold text-[14px] uppercase tracking-[0.1em] text-charcoal/50 dark-flip-muted block mb-2">Status</label>
          <div className="relative">
      ...
      <div className="flex items-center gap-4 pt-2">
        <button type="submit" disabled={status === 'loading'}
          className="inline-flex items-center gap-2.5 bg-action hover:bg-action-hover disabled:opacity-60 disabled:cursor-not-allowed text-cream font-display font-bold text-[14px] uppercase tracking-[0.12em] px-8 py-3.5 rounded-full transition-all shadow-[0_4px_20px_rgba(162,46,41,0.22)]">
          {status === 'loading' ? (
            <>
