...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`inline-flex items-center font-inter text-[14px] uppercase tracking-[0.12em] px-2.5 py-1 rounded-full ${STATUS_STYLE[status] ?? 'bg-charcoal/8 text-charcoal/40'}`}>
      {status}
    </span>
  )
}
...
export function PipelineBoard({ attractions, role }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-1.5">
        {STATUS_TABS.map(tab => (
          <button
            ...
            className={`flex items-center gap-1.5 font-inter text-[14px] uppercase tracking-[0.12em] px-3.5 py-2 rounded-full border transition-all ${
              activeTab === tab
      ...
      {/* Results count */}
      <p className="font-inter text-[14px] uppercase tracking-[0.14em] text-charcoal/30 dark-flip-muted">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-line dark-flip-border bg-sand dark-flip-surf">
                  <th className="text-left px-5 py-3.5 font-inter text-[14px] uppercase tracking-[0.14em] text-charcoal/40 dark-flip-muted">Attraction</th>
                  <th className="text-left px-4 py-3.5 font-inter text-[14px] uppercase tracking-[0.14em] text-charcoal/40 dark-flip-muted hidden md:table-cell">Country</th>
                  <th className="text-left px-4 py-3.5 font-inter text-[14px] uppercase tracking-[0.14em] text-charcoal/40 dark-flip-muted">Status</th>
                  <th className="text-left px-4 py-3.5 font-inter text-[14px] uppercase tracking-[0.14em] text-charcoal/40 dark-flip-muted hidden lg:table-cell">Verified</th>
                  <th className="text-right px-5 py-3.5 font-inter text-[14px] uppercase tracking-[0.14em] text-charcoal/40 dark-flip-muted">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.slice(0, 200).map((a, i) => (
                  <tr key={a._id}
                    ...
                    <td className="px-5 py-3.5">
                      <div>
                        <p className="font-display font-semibold text-[14px] text-charcoal dark-flip-text leading-snug">
                          {a.name}
                        </p>
                        {a.type?.[0] && (
                          <p className="font-inter text-[14px] text-charcoal/30 dark-flip-muted mt-0.5 truncate max-w-[200px]">{a.type[0]}</p>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3.5 hidden lg:table-cell">
                      <p className="font-inter text-[14px] text-charcoal/35 dark-flip-muted">
                        {a.lastVerifiedDate ?? '—'}
                      </p>
                    </td>
                          ...
                          <>
                            ...
                            {(a.contentStatus === 'Draft' || a.contentStatus === 'Incomplete') && (
                              <button onClick={() => changeStatus(a._id, 'Verified')}
                                className="font-inter text-[14px] uppercase tracking-[0.1em] text-moss-700 hover:text-moss-600 border border-moss-200 hover:border-moss-400 px-2.5 py-1 rounded-lg transition-all whitespace-nowrap">
                                Submit
                              </button>
                            )}
                            ...
                            {role === 'admin' && a.contentStatus === 'Verified' && (
                              <>
                                <button onClick={() => changeStatus(a._id, 'Published')}
                                  className="font-inter text-[14px] uppercase tracking-[0.1em] text-gold-700 hover:text-gold-600 border border-gold-200 hover:border-gold-400 px-2.5 py-1 rounded-lg transition-all">
                                  Publish
                                </button>
                                <button onClick={() => changeStatus(a._id, 'Draft')}
                                  className="font-inter text-[14px] uppercase tracking-[0.1em] text-charcoal/40 hover:text-charcoal/70 border border-line hover:border-charcoal/30 px-2.5 py-1 rounded-lg transition-all">
                                  Return
                                </button>
                              </>
                            )}
                            ...
                            {role === 'admin' && a.contentStatus === 'Published' && (
                              <button onClick={() => changeStatus(a._id, 'Needs Update')}
                                className="font-inter text-[14px] uppercase tracking-[0.1em] text-crimson/70 hover:text-crimson border border-crimson/20 hover:border-crimson/40 px-2.5 py-1 rounded-lg transition-all whitespace-nowrap">
                                Flag
                              </button>
                            )}
      ...
      {filtered.length > 200 && (
        <p className="font-inter text-[14px] uppercase tracking-[0.12em] text-charcoal/30 dark-flip-muted text-center">
