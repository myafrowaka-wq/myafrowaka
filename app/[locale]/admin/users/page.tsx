...
const ROLE_STYLE: Record<UserRole, string> = {
  ...
  visitor:         'text-cream/55 border-white/10 bg-white/3',
}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
        {ASSIGNABLE_ROLES.map(role => (
          <div key={role} className="bg-white/5 border border-white/8 rounded-xl p-4">
            ...
            <p className="font-sans text-[14px] text-cream/55 leading-relaxed">{ROLE_DESCRIPTIONS[role]}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white/5 border border-white/8 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <p className="font-sans text-sm text-cream/55">No users yet.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/8">
                  <th className="text-left px-5 py-4 font-sans text-[14px] uppercase tracking-[0.14em] text-cream/55">User</th>
                  <th className="text-left px-4 py-4 font-sans text-[14px] uppercase tracking-[0.14em] text-cream/55 hidden md:table-cell">Joined</th>
                  <th className="text-left px-4 py-4 font-sans text-[14px] uppercase tracking-[0.14em] text-cream/55">Role</th>
                  <th className="text-left px-5 py-4 font-sans text-[14px] uppercase tracking-[0.14em] text-cream/55">Change to</th>
                </tr>
              </thead>
              <tbody>
                {users.map(u => (
                  <tr key={u._id} className="border-b border-white/6 last:border-0 hover:bg-white/3 transition-colors">
                    <td className="px-5 py-4">
                      ...
                      <p className="font-sans text-[14px] text-cream/55 mt-0.5">{u.userEmail}</p>
                    </td>
                    <td className="px-4 py-4 hidden md:table-cell">
                      <p className="font-sans text-[14px] text-cream/55">{formatDate(u.createdAt)}</p>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center flex-wrap gap-1.5">
                        {saving === u._id ? (
                          <svg className="w-4 h-4 animate-spin text-cream/55" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                          </svg>
                        ) : (
