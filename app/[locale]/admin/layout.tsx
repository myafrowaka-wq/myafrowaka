...
const NAV = [
  { label: 'Overview',    href: '/admin',             adminOnly: false, icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
  { label: 'Attractions', href: '/admin/attractions',  adminOnly: false, icon: 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z' },
  { label: 'SEO Audit',   href: '/admin/seo',          adminOnly: false, icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
  { label: 'Users',       href: '/admin/users',        adminOnly: true,  icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
]

export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  const session = await auth()
  const locale = await getLocale()
  if (!session) redirect({ href: '/login?next=/admin', locale })
  ...
  if (!atLeast(role, 'contributor')) redirect({ href: '/', locale })
        ...
        <div className="grid lg:grid-cols-[220px_1fr] gap-6 lg:gap-10 items-start">
          ...
          <aside className="lg:sticky lg:top-6 space-y-3">
            ...
            <div className="flex items-center gap-2.5 px-2 mb-6">
              ...
              <span className="font-sans text-[14px] uppercase tracking-[0.16em] text-cream/55 border border-cream/15 px-2 py-0.5 rounded-full">
                Admin
              </span>
            </div>
            ...
            <div className="border-t border-white/8 pt-3 mt-3 space-y-0.5">
              <Link href="/studio" target="_blank"
                className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-sans text-[14px] text-cream/55 hover:text-cream/70 hover:bg-white/5 transition-all">
            ...
            <div className="border-t border-white/8 pt-4 mt-2 flex items-center gap-2.5 px-1">
              ...
              <div className="min-w-0">
                ...
                <p className="font-sans text-[14px] uppercase tracking-[0.12em] text-gold-400/70">{role}</p>
              </div>
            </div>
          </aside>
