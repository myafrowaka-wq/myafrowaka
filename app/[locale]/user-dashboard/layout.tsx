import { redirect, getPathname } from '@/i18n/navigation'
import { getLocale } from 'next-intl/server'
import { auth, signOut } from '@/auth'
import { client } from '@/sanity/lib/client'
import { DashboardSidebar } from '@/components/DashboardSidebar'
import type { UserRole } from '@/types/next-auth'

export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  const session = await auth()
  if (!session?.user) redirect({ href: '/login', locale: await getLocale() })

  const { user } = session!
  const role = (user.role ?? 'subscriber') as UserRole

  const savedCount = await client
    .fetch<number>(`count(*[_type == "savedAttraction" && userId == $userId])`, { userId: user.id })
    .catch(() => 0)

  const initials = (user.name ?? user.email ?? 'U')
    .split(' ')
    .map((w: string) => w[0])
    .join('')
    .slice(0, 2)
    .toUpperCase()

  async function handleSignOut() {
    'use server'
    // Session 5.3 second look-through — signOut()'s own redirect bypasses
    // next-intl entirely (same class of gap as the sign-in redirectTo fix in
    // app/[locale]/login/page.tsx), so a bare '/' here silently dropped a
    // French/Portuguese visitor back to the English homepage on sign-out.
    await signOut({ redirectTo: getPathname({ href: '/', locale: await getLocale() }) })
  }

  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-cream dark-flip-bg">
      <DashboardSidebar
        role={role}
        userName={user.name ?? ''}
        userEmail={user.email ?? ''}
        initials={initials}
        savedCount={savedCount}
        signOutAction={handleSignOut}
      />
      <div className="flex-1 min-w-0">
        {children}
      </div>
    </div>
  )
}
