...
// Owner review (2026-09-06) — real bug: this button rendered
// unconditionally regardless of whether Google is actually usable.
// auth.ts already computes this exact check to decide whether to
// register the provider at all (`hasGoogle`); this page just never
// asked the same question, so clicking the button when the answer was
// "no" hit NextAuth's generic /api/auth/signin fallback and bounced
// straight back to this page with a callbackUrl query param and no
// explanation — confirmed live: no visible error, no redirect to
// Google, just a silent round-trip. Hiding the button when it can't
// work is the honest fix; showing it and hoping the visitor doesn't
// click during setup is what caused the confusion in the first place.
    ...
    <div className="min-h-screen bg-cream dark-flip-bg flex">
      ...
      <div className="w-full lg:w-1/2 flex items-start justify-center px-6 py-16 overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          {/* Google — only rendered when it can actually work; see the
              hasGoogle comment above. */}
          {hasGoogle && (
            <div className="mb-6">
              <form action={async () => {
                'use server'
          ...
          {/* This divider only makes sense when there's something above it
              to divide from — with Google hidden, the email form is the
              first option, not an alternative "or" to one. */}
          {hasGoogle && (
