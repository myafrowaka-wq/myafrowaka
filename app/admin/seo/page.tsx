  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            <div key={s.label} className="bg-white/5 border border-white/8 rounded-2xl p-4 text-center">
              ...
              <p className="font-inter text-[14px] uppercase tracking-[0.14em] text-cream/30 mt-1">{s.label}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-4">
        <p className="font-inter text-[14px] uppercase tracking-[0.14em] text-cream/30">Checks:</p>
        {[
        ...
        ].map(l => <span key={l.label} className="font-inter text-[14px] text-cream/40">{l.label}</span>)}
        <div className="flex items-center gap-2 ml-auto">
          <span className="flex items-center gap-1.5"><span className="inline-block w-2 h-2 rounded-full bg-moss-400"/><span className="font-inter text-[14px] text-cream/35">OK</span></span>
          <span className="flex items-center gap-1.5"><span className="inline-block w-2 h-2 rounded-full bg-gold-400"/><span className="font-inter text-[14px] text-cream/35">Too long</span></span>
          <span className="flex items-center gap-1.5"><span className="inline-block w-2 h-2 rounded-full bg-crimson"/><span className="font-inter text-[14px] text-cream/35">Missing</span></span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {STATUS_TABS.map(s => (
            <button key={s} onClick={() => setTab(s)}
              className={`font-inter text-[14px] uppercase tracking-[0.1em] px-3 py-1.5 rounded-full border transition-all ${
                tab === s
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <select value={scoreFilter} onChange={e => setScoreFilter(e.target.value as typeof scoreFilter)}
            className="bg-white/6 border border-white/12 rounded-xl px-3 py-2 font-inter text-[14px] uppercase tracking-[0.1em] text-cream/50 focus:outline-none">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/8">
                  <th className="text-left px-5 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25">Attraction</th>
                  <th className="text-left px-4 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25 hidden md:table-cell">Status</th>
                  <th className="text-center px-4 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25">Score</th>
                  <th className="text-center px-3 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25 hidden sm:table-cell">Title</th>
                  <th className="text-center px-3 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25 hidden sm:table-cell">Desc</th>
                  <th className="text-center px-3 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25 hidden lg:table-cell">KW</th>
                  <th className="text-center px-3 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25 hidden lg:table-cell">Body</th>
                  <th className="text-center px-3 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25 hidden lg:table-cell">Date</th>
                  <th className="text-right px-5 py-3.5 font-inter text-[14px] uppercase tracking-[0.16em] text-cream/25">Edit</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(a => {
                  ...
                  return (
                    <tr key={a._id} className="border-b border-white/6 last:border-0 hover:bg-white/3 transition-colors">
                      <td className="px-5 py-3.5">
                        ...
                        {a.country && (
                          <p className="font-inter text-[14px] text-cream/25 mt-0.5 uppercase tracking-[0.1em]">{a.country.name}</p>
                        )}
                      </td>
                      <td className="px-4 py-3.5 hidden md:table-cell">
                        <span className={`font-inter text-[14px] uppercase tracking-[0.1em] px-2 py-0.5 rounded-full border ${STATUS_STYLE[a.contentStatus] ?? 'text-cream/30 border-white/10'}`}>
                          {a.contentStatus}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-center">
                        <span className={`font-inter font-bold text-[14px] ${scoreColor}`}>{h.score}/5</span>
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <a href={`/studio/structure/attraction;${a._id}`} target="_blank"
                          ...
                          className="font-inter text-[14px] uppercase tracking-[0.1em] text-gold-400/60 hover:text-gold-400 border border-gold-400/20 hover:border-gold-400/40 px-2.5 py-1 rounded-lg transition-all inline-block">
                          Edit
                        </a>
                      </td>
                    </tr>
            ...
            {filtered.length === 250 && (
              <div className="px-5 py-3 border-t border-white/6 text-center">
                <p className="font-inter text-[14px] uppercase tracking-[0.1em] text-cream/20">Showing first 250 results. Use filters to narrow down.</p>
              </div>
            )}
          </div>
        )}
