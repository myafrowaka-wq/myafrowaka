    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href="/user-dashboard/admin/new-attraction"
            className="inline-flex items-center gap-2 bg-crimson hover:bg-crimson/85 text-cream font-inter text-[10px] uppercase tracking-[0.12em] px-4 py-2.5 rounded-xl transition-all">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4"/></svg>
            Add Attraction
          </Link>
          <Link href="/user-dashboard/admin/new-post"
            className="inline-flex items-center gap-2 bg-white/8 hover:bg-white/15 border border-white/12 text-cream font-inter text-[10px] uppercase tracking-[0.12em] px-4 py-2.5 rounded-xl transition-all">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-gradient-to-br from-gold-400/12 to-gold-400/4 border border-gold-400/20 rounded-2xl p-5 relative overflow-hidden">
          <p className="font-inter text-[8px] uppercase tracking-[0.14em] text-gold-400/60 mb-1">Published</p>
          <p className="font-display font-bold text-gold-400 mb-0.5" style={{ fontSize: 'clamp(28px,3vw,42px)', letterSpacing: '-0.02em' }}>
            {pubPct}%
          </p>
          <p className="font-inter text-[9px] text-cream/30">{published} of {total} live</p>
        ...
        <div className="bg-gradient-to-br from-moss-400/12 to-moss-400/4 border border-moss-400/20 rounded-2xl p-5 relative overflow-hidden">
          <p className="font-inter text-[8px] uppercase tracking-[0.14em] text-moss-400/60 mb-1">Ready to Publish</p>
          <p className="font-display font-bold text-moss-400 mb-0.5" style={{ fontSize: 'clamp(28px,3vw,42px)', letterSpacing: '-0.02em' }}>
            {verified}
          </p>
          <p className="font-inter text-[9px] text-cream/30">Verified guides</p>
        ...
        <div className="bg-gradient-to-br from-crimson/10 to-crimson/3 border border-crimson/20 rounded-2xl p-5 relative overflow-hidden">
          <p className="font-inter text-[8px] uppercase tracking-[0.14em] text-crimson/60 mb-1">SEO Coverage</p>
          <p className="font-display font-bold text-cream mb-0.5" style={{ fontSize: 'clamp(28px,3vw,42px)', letterSpacing: '-0.02em' }}>
            {seoScore}%
          </p>
          <p className="font-inter text-[9px] text-cream/30">{seoStats.withMeta}/{seoStats.published} with meta</p>
          <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/5">
        ...
        <div className={`border rounded-2xl p-5 relative overflow-hidden ${needsUpd > 0 ? 'bg-gradient-to-br from-crimson/15 to-crimson/5 border-crimson/30' : 'bg-white/5 border-white/8'}`}>
          <p className="font-inter text-[8px] uppercase tracking-[0.14em] text-cream/40 mb-1">Needs Update</p>
          ...
          <p className="font-inter text-[9px] text-cream/30">Flagged for review</p>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white/5 border border-white/8 rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
            <div>
              <p className="font-inter text-[9px] uppercase tracking-[0.16em] text-moss-400/70 mb-0.5">Ready to Publish</p>
              <p className="font-display font-semibold text-cream text-[13px]">Publishing Queue</p>
            </div>
            <Link href="/admin/attractions"
              className="font-inter text-[9px] uppercase tracking-[0.12em] text-gold-400/60 hover:text-gold-400 transition-colors">
              View all
            </Link>
          </div>
          {publishingQueue.length === 0 ? (
            <div className="px-5 py-8 text-center">
              <p className="font-sans text-[13px] text-cream/30">No verified attractions waiting.</p>
              <p className="font-inter text-[10px] text-cream/20 mt-1">Verify a draft to see it here.</p>
            </div>
          ) : (
              ...
              {publishingQueue.map((a, i) => (
                <div key={i} className="flex items-center justify-between px-5 py-3.5 group">
                  <div className="min-w-0 mr-3">
                    <p className="font-display font-semibold text-[13px] text-cream/80 group-hover:text-cream transition-colors truncate"
                      style={{ letterSpacing: '-0.008em' }}>
                      {a.name}
                    </p>
                    {a.country && (
                      <p className="font-inter text-[9px] text-cream/30 mt-0.5">{a.country.name}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="font-inter text-[8px] text-cream/25">{timeAgo(a._updatedAt)}</span>
                    <Link href={`/attractions/${a.slug}`} target="_blank"
                      className="font-inter text-[8px] uppercase tracking-[0.1em] bg-moss-400/15 hover:bg-moss-400/25 text-moss-400 border border-moss-400/25 px-2.5 py-1 rounded-full transition-colors">
                      Preview
                    </Link>
                  </div>
                </div>
        ...
        <div className="bg-white/5 border border-white/8 rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
            <div>
              <p className="font-inter text-[9px] uppercase tracking-[0.16em] text-cream/35 mb-0.5">Latest Changes</p>
              <p className="font-display font-semibold text-cream text-[13px]">Recent Activity</p>
            </div>
          </div>
          <div className="divide-y divide-white/5">
            {recentActivity.map((a, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-sans text-[12px] text-cream/75 truncate">{a.name}</p>
                  <p className="font-inter text-[9px] text-cream/25 mt-0.5">
                    {a.country?.name ?? ''}{a.country ? ' · ' : ''}<span className={STATUS_TEXT[a.contentStatus] ?? 'text-cream/20'}>{a.contentStatus}</span>
                  </p>
                </div>
                <span className="font-inter text-[8px] text-cream/20 shrink-0">{timeAgo(a._updatedAt)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white/5 border border-white/8 rounded-2xl p-6">
          <p className="font-inter text-[9px] uppercase tracking-[0.16em] text-cream/35 mb-5">Geographic Coverage</p>
          <div className="space-y-4">
            {geoRows.map(row => (
              <div key={row.region}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="font-sans text-[12px] text-cream/65">{row.region}</span>
                  <div className="flex items-center gap-2">
                    <span className="font-inter text-[10px] text-cream/35">{row.pub}/{row.total}</span>
                    <span className="font-inter text-[10px] text-gold-400/80 w-8 text-right">{row.pct}%</span>
                  </div>
                </div>
        ...
        <div className="bg-white/5 border border-white/8 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={s.status} className="flex items-center gap-3">
                ...
                <span className="font-inter text-[10px] text-cream/50 w-8 text-right shrink-0">{s.cnt}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-white/8 grid grid-cols-2 gap-2">
            <Link href="/admin/attractions"
              ...
              <p className="font-inter text-[8px] uppercase tracking-[0.1em] text-gold-400/50 mb-1">Pipeline</p>
              <p className="font-display font-semibold text-cream text-[12px] group-hover:text-gold-300 transition-colors">Manage Attractions</p>
            </Link>
            {session.user?.role === 'admin' && (
              <Link href="/admin/users"
                className="bg-white/5 hover:bg-white/10 border border-white/8 rounded-xl p-3 group transition-all">
                <p className="font-inter text-[8px] uppercase tracking-[0.1em] text-gold-400/50 mb-1">Team</p>
                <p className="font-display font-semibold text-cream text-[12px] group-hover:text-gold-300 transition-colors">Manage Users</p>
              </Link>
            )}
          </div>
        </div>
            ...
            {recentUsers.map((u, i) => (
              <div key={i} className="flex items-center gap-3 bg-white/4 border border-white/8 rounded-xl p-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gold-400/30 to-gold-600/20 border border-gold-400/20 flex items-center justify-center shrink-0">
                  <span className="font-display font-bold text-[11px] text-gold-400">
                    {u.userName.charAt(0).toUpperCase()}
                  </span>
                </div>
                <div className="min-w-0">
                  <p className="font-sans text-[12px] text-cream/75 truncate">{u.userName}</p>
                  <span className={`font-inter text-[8px] uppercase tracking-[0.1em] ${
