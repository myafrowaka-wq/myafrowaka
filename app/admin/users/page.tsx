      ...
      <div className="grid sm:grid-cols-3 gap-3">
        {([
          ...
          <div key={r.role} className="bg-white/5 border border-white/8 rounded-xl p-4">
            <span className={`inline-block font-sans text-[14px] uppercase tracking-[0.12em] px-2 py-0.5 rounded-full border mb-2 ${ROLE_STYLE[r.role]}`}>
              {r.role}
            </span>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/8">
                  ...
                  <th className="text-right px-5 py-4 font-sans text-[14px] uppercase tracking-[0.14em] text-cream/25">Change</th>
                </tr>
              </thead>
              <tbody>
                {users.map(u => (
                  <tr key={u._id} className="border-b border-white/6 last:border-0 hover:bg-white/3 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span className={`inline-block font-sans text-[14px] uppercase tracking-[0.12em] px-2.5 py-1 rounded-full border ${ROLE_STYLE[u.role]}`}>
                        {u.role}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                          ...
                          <>
                            {u.role !== 'contributor' && (
                              <button onClick={() => changeRole(u._id, 'contributor')}
                                className="font-sans text-[14px] uppercase tracking-[0.1em] text-moss-400/70 hover:text-moss-400 border border-moss-400/20 hover:border-moss-400/40 px-2.5 py-1 rounded-lg transition-all">
                                Contributor
                              </button>
                            )}
                            {u.role !== 'admin' && (
                              <button onClick={() => changeRole(u._id, 'admin')}
                                className="font-sans text-[14px] uppercase tracking-[0.1em] text-gold-400/70 hover:text-gold-400 border border-gold-400/20 hover:border-gold-400/40 px-2.5 py-1 rounded-lg transition-all">
                                Admin
                              </button>
                            )}
                            {u.role !== 'visitor' && (
                              <button onClick={() => changeRole(u._id, 'visitor')}
                                className="font-sans text-[14px] uppercase tracking-[0.1em] text-cream/25 hover:text-cream/50 border border-white/10 hover:border-white/20 px-2.5 py-1 rounded-lg transition-all">
                                Visitor
                              </button>
                            )}
                          </>
